...
- connector registry state is now readable at `/api/connectors`, and operator tokens can trigger `/api/connectors/sync` or `/api/connectors/sync-all` remotely through the same hosted-alpha surface
