...
- hosted operators can now inspect `GET /api/research-runs`, record `POST /api/research-steps/review`, and trigger `POST /api/research/resume`, so the same reviewed hosted run can move from remote execution to final filing without dropping back to a local shell
...
- `GET /api/research-runs` exposes research-run readiness, reconcile blockers, assignment summaries, and step-level execution or review state, while `POST /api/research-steps/review` and `POST /api/research/resume` let hosted reviewers and operators finish the research loop over HTTP with the same checkpoint-backed runtime as the local CLI
...
- the same dashboard now includes a Research Operator panel with research-run readiness, assignment metadata, reconcile blockers, step-output links, reviewer/operator step-review actions, and operator-only finalize or profile-rerun actions
...
- `control-plane status` now includes a hosted hardening readout for long-lived tokens, broad operator tokens, permissive trust policy, stale active workers, queued hosted work, and high-severity notifications; `GET /api/status` returns the same report without exposing raw tokens or token hashes
...
- `GET /api/status` includes the hosted hardening report from the local status command, so remote operators can inspect token expiry posture, trust-policy breadth, worker staleness, queue backlog, and high-severity notification risks before dispatching more work
