...
The dashboard is intentionally thin. It reads the same review queue and review-action state you already use through the CLI, then layers in graph-overview data from `.cognisync/graph.json`, source coverage from `.cognisync/sources.json`, compile health from lint and compile-plan state, recent change summaries, run history from `.cognisync/runs/`, research-run readiness and assignment state, queued-job history from `.cognisync/jobs/`, worker ownership from `.cognisync/jobs/workers.json`, sync audit history from `.cognisync/sync/`, connector definitions from `.cognisync/connectors.json`, workspace access state from `.cognisync/access.json`, collaboration state from `.cognisync/collaboration.json`, and operator notifications from `.cognisync/notifications.json`. It also writes static graph-node, run-detail, run-timeline, concept-graph, job-detail, sync-detail, connector-detail, and artifact-preview pages plus lightweight browser-side filters, so operators can drill into the current graph, source mix, change ledger, queued work, connector registry, research blockers, access roster, collaboration queue, and sync handoffs without leaving the file-native workflow. When served locally, the same surface can accept concepts, dismiss or reopen queue items, apply backlinks, file conflicts, resolve merge candidates, request artifact review, add collaboration comments, record approvals or requested changes, resolve collaboration threads, review research-step outputs, finalize ready research runs, intentionally rerun a research profile, run the next queued job, sync one registered connector, or sync the unsynced portion of the whole connector registry. Those live actions now run as an explicit workspace actor, with editor-or-reviewer-or-operator access for collaboration requests and comments, reviewer-or-operator access for approvals, change requests, and research-step review, and operator-only access for research resume, job, and connector mutations. The filesystem stays canonical and the UI remains a control layer rather than a second source of truth.
...
- `GET /api/research-runs` exposes hosted research-run readiness, reconcile blockers, assignment summaries, and step-level execution or review state over the same token-backed surface
...
- `POST /api/research-steps/review` lets reviewer or operator tokens record the same checkpoint-backed research-step decisions the local CLI writes, while deriving the reviewer identity from the authenticated principal instead of trusting caller-supplied names
- `POST /api/research/resume` lets operator tokens trigger the same checkpoint-finalize or adapter-rerun path as `cognisync research --resume`, so a hosted run can be closed out without a local shell
